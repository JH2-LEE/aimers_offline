...
def make_train_data(
    ...
    input_data = np.empty(
        (
            ...
            train_size,
            1 + product.shape[1] + time_series.shape[0] + 3,
        )
    )
...
def make_predict_data(data, train_size=CFG["TRAIN_WINDOW_SIZE"]):
    ...
    input_data = np.empty(
        (num_rows, train_size, 1 + product.shape[1] + time_series.shape[0] + 3)
    )
...
# torch.save(infer_model.state_dict(), "./patchtst_submit_0823_ver1.pth")
...
infer_model.load_state_dict(torch.load("./patchtst_submit_0823_ver1.pth"))
...
submit.iloc[:, 1:] = pred
submit.to_csv("./patchtst_submit_0823_ver1.csv", index=False)
