...
nprice_big = product_features[["NormalizedPrice_대분류"]].to_numpy()

product = np.column_stack((sales_mean_scaled, prod_num_scaled, nprice_big))
...
time_series = np.stack((sale_info, salary, holiday, month, dayofweek))
...
def make_train_data(
    ...
    input_data = np.empty(
        (
            ...
            train_size,
            1 + product.shape[1] + time_series.shape[0] + 2 + 1,
        )
    )
        ...
        for j in range(len(sales_data) - window_size + 1):
            ...
            temp_data = np.concatenate(
                (
                    ...
                    sale_event_window[:train_size].reshape((-1, 1)),
...
def make_predict_data(data, train_size=CFG["TRAIN_WINDOW_SIZE"]):
    ...
    input_data = np.empty(
        (num_rows, train_size, 1 + product.shape[1] + time_series.shape[0] + 2 + 1)
    )
...
def train(model, optimizer, scheduler, train_loader, val_loader, device):
    ...
    for epoch in range(1, CFG["EPOCHS"] + 1):
        ...
        for X, Y in tqdm(iter(train_loader)):
            ...
            loss = (1 - CFG["LAMBDA"]) * criterion(output, Y) + CFG["LAMBDA"] * psfa(
                output, Y
            )
...
torch.save(infer_model.state_dict(), "./patchtst_submit_0821_ver2.pth")
...
infer_model.load_state_dict(torch.load("./patchtst_submit_0821_ver1.pth"))

# 성능확인용(추가)
test_dataset_kuka = CustomDataset(test_input_kuka, None)
test_loader_kuka = DataLoader(
    test_dataset_kuka, batch_size=CFG["BATCH_SIZE"], shuffle=False, num_workers=0
)
...
def PSFA(pred, target):
    ...
    for cat in range(5):
        ...
        for day in range(21):
            ...
            diffs = np.where(
                denominator != 0, np.abs(target_values - pred_values) / denominator, 0
            )

            if total_sell != 0:
                sell_weights = target_values / total_sell  # Item별 day 총 판매량 내 비중
            else:
                sell_weights = np.ones_like(target_values) / len(
                    ids
                )  # 1 / len(ids)로 대체
...
for idx in range(len(pred_kuka)):
    pred_kuka[idx, :] = (
        pred_kuka[idx, :] * (scale_max_dict[idx] - scale_min_dict[idx])
        + scale_min_dict[idx]
    )

# 결과 후처리
pred_kuka = np.round(pred_kuka, 0).astype(int)
# print("ddd", pred_kuka.shape)
