...
idx_number = train_data[cond].index
...
product_features = product_features[cond].reset_index(drop=True)
...
class PSFALoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # inputs와 targets를 같은 장치로 이동
        inputs = inputs.to(targets.device)

        share_denominator = torch.sum(targets, dim=0)
        share_denominator[share_denominator == 0] = 1  # 나눠지게 만들기
        share = targets / share_denominator
        error_demoninator = torch.max(inputs, targets)
        error_demoninator[error_demoninator == 0] = 1  # 나눠지게 만들기
        error = torch.abs(inputs - targets) / error_demoninator
        metric = error * share
        loss = torch.mean(torch.sum(metric, dim=1))
        return loss
    ...
    for epoch in range(1, CFG["EPOCHS"] + 1):
        ...
        if best_loss > val_loss:  # val loss가 가장 작은 모델을 저장
            ...
            torch.save(
                ...
                "/root/workspace/minkyung/ckpt/patchtst_mall1_submit_0917_1_epoch{}.pth".format(epoch),
            )
    return best_model
...
torch.save(infer_model.state_dict(), "/root/workspace/minkyung/ckpt/patchtst_mall1_submit_0917_1_final.pth")
