...
# The above code is retrieving the index of the `train_data` object.
train_data = train_data[cond]

idx_number = train_data.index
output_kuka = train_data.iloc[:, -CFG["PREDICT_SIZE"] :].to_numpy()
product_features = pd.read_csv("../../data/features2.csv").drop(
...
infer_model.load_state_dict(torch.load("./ckpt/patchtst_submit_0916_1_epoch13.pth"), strict=False)
...
submit = submit.iloc[idx_number]
submit.iloc[:, 1:] = pred
submit.to_csv("../submit/patchtst_submit_0916_1_epoch13.csv", index=False)
