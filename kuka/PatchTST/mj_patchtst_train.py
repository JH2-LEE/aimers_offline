...
train_data = pd.read_csv("/root/workspace/kuka/dataset/train.csv").drop(columns=["ID", "제품"])
# cond = train_data["쇼핑몰"]
output_kuka = train_data.iloc[:, -CFG["PREDICT_SIZE"] :].to_numpy()
product_features = pd.read_csv("/root/workspace/kuka/dataset/features_old.csv").drop(
    columns=["Unnamed: 0","ID", "제품", "대분류", "중분류", "소분류", "브랜드"]
)
time_series_features = pd.read_csv("/root/workspace/kuka/dataset/total_dates_scaling_old.csv").drop(
