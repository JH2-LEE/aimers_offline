...
train_data = pd.read_csv("/content/drive/MyDrive/data/train.csv").drop(columns=["ID", "제품"])
cond = (train_data["쇼핑몰"] != "S001-00001")
train_data = train_data[cond] #####
output_kuka = train_data.iloc[:, -CFG["PREDICT_SIZE"] :].to_numpy()
product_features = pd.read_csv("/content/drive/MyDrive/data/features4.csv").drop(
    columns=["Unnamed: 0","ID", "제품", "대분류", "중분류", "소분류", "브랜드","쇼핑몰"]
)
product_features = product_features[cond]
print(train_data)
print(product_features)
time_series_features = pd.read_csv("/content/drive/MyDrive/data/total_dates_scaling2.csv").drop(
    ...
    for epoch in range(1, CFG["EPOCHS"] + 1):
        ...
        if best_loss > val_loss:  # val loss가 가장 작은 모델을 저장
            ...
            torch.save(
              ...
              "./ckpt/patchtst_submit_0916_1_epoch{}.pth".format(epoch),
            )
...
torch.save(infer_model.state_dict(), "./ckpt/patchtst_submit_0916_1_final.pth")
