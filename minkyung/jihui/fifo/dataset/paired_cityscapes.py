...
class Pairedcityscapes(data.Dataset):
    ...
    def __init__(
        ...
        for name in self.img_ids:
            trg_img_file = osp.join(
                self.trg_root, "leftImg8bit/%s/%s" % (self.set, name[:-21] + ".png")
            )
            src_img_file = osp.join(
                self.src_root,
                # "./leftImg8bit_foggyDBF/%s/%s" % (self.set, name),
                "./Cityscapes/leftImg8bit_foggyDBF/%s/%s" % (self.set, name),
            )
            label_file = osp.join(
                self.src_root,
                "./Cityscapes/gtFine/%s/%s"
                % (self.set, name[:-32] + "gtFine_labelIds.png"),
            )
            self.files.append(
                {
    ...
    def __getitem__(self, index):
        ...
        # resize
        w, h = src_image.size
        src_image, trg_image, label = self._apply_transform(
            src_image, trg_image, label, scale=0.8
        )
