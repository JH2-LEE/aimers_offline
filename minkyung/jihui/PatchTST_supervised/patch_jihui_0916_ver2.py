...
CFG = {
    "TRAIN_WINDOW_SIZE": 104,  # 128일치로 학습
...
train_data = pd.read_csv("./data_phase3/train.csv").drop(columns=["ID", "제품"])
output_kuka = train_data.iloc[:, -CFG["PREDICT_SIZE"] :].to_numpy()
product_features = pd.read_csv("./data_phase3/features2.csv").drop(
    columns=["Unnamed: 0", "ID", "제품", "대분류", "중분류", "소분류", "브랜드"]
)
time_series_features = pd.read_csv("./data_phase3/total_dates_scaling.csv").drop(
...
scaler3.fit(product_features[[" 쇼핑몰개수"]])
mall_num_scaled = scaler3.transform(product_features[[" 쇼핑몰개수"]])

nprice_mid = product_features[["NormalizedPrice_중분류"]].to_numpy()

product = np.column_stack((sales_mean_scaled, prod_num_scaled, mall_num_scaled, nprice_mid))
        ...
        for j in range(len(sales_data) - window_size + 1):
            ...
            temp_data = np.concatenate(
                (
                    ...
                    # price_window[:train_size].reshape((-1, 1)),
...
def make_train_data_new(
    ...
    input_data = np.empty((num_rows * adjusted_size, train_size, 12))
    target_data = np.empty((num_rows * adjusted_size, predict_size, 12))
    ...
    for i in tqdm(range(num_rows)):
        ...
        for j in range(0, len(sales_data) - window_size + 1, STEP_SIZE):
            ...
            target_data[i * adjusted_size + j // STEP_SIZE] = temp_target
...
train_input, train_target = make_train_data_new(train_data)  # train val total

# import pickle

# # Save
# with open("./make_train_data_new/train_input.pkl","wb") as f:
#     pickle.dump(train_input, f)
# with open("./make_train_data_new/train_target.pkl","wb") as f:
...
def train(model, optimizer, scheduler, train_loader, val_loader, device):
    ...
    for epoch in range(11, CFG["EPOCHS"] + 1):
        model.train()
        train_loss = []
        # train_mae = []
        for X, Y in tqdm(iter(train_loader), dynamic_ncols=True):
            ...
            loss = CFG["LAMBDA1"] * criterion(output, Y) + CFG["LAMBDA1"] * mae(output, Y) + CFG["LAMBDA2"] * psfa(
        ...
        if best_loss > val_loss:
            best_loss = val_loss
            best_model = model
            torch.save(best_model.state_dict(), "./patchtst_submit_0916_ver2_epoch{}.pth".format(epoch))
            print("Model Saved")
    return best_model
...
model.load_state_dict(torch.load("./patchtst_submit_0916_ver2_epoch10.pth"))
...
torch.save(infer_model.state_dict(), "./patchtst_submit_0916_ver2_final2.pth")
...
# infer_model.load_state_dict(torch.load("./patchtst_submit_0916_ver2_epoch10.pth"))
...
# submit = pd.read_csv("./data_phase3/sample_submission.csv")
# submit.iloc[:, 1:] = pred
# submit.to_csv("./patchtst_submit_0916_ver2_epoch10.csv", index=False)
