...
def train(model, optimizer, scheduler, train_loader, val_loader, device):
    ...
    for epoch in range(1, CFG["EPOCHS"] + 1):
        ...
        for X, Y in tqdm(iter(train_loader), dynamic_ncols=True):
            ...
            loss = CFG["LAMBDA1"] * criterion(output_sales, Y_sales) + CFG[
                "LAMBDA2"
            ] * psfa(output_sales, Y_sales)
...
# torch.save(infer_model.state_dict(), "./patchtst_submit_0822_ver2.pth")
...
infer_model.load_state_dict(torch.load("./patchtst_submit_0822_ver2.pth"))
...
submit.iloc[:, 1:] = pred
submit.to_csv("./patchtst_submit_0822_ver2.csv", index=False)
