...
# PatchTST
parser.add_argument(
    "--fc_dropout", type=float, default=0.2, help="fully connected dropout"
)
...
def train(model, optimizer, scheduler, train_loader, val_loader, device):
    ...
    for epoch in range(1, CFG["EPOCHS"] + 1):
        ...
        if best_loss > val_loss:
            best_loss = val_loss
            best_model = model
            torch.save(best_model.state_dict(), "./patchtst_submit_0916_epoch{}.pth".format(epoch))
            print("Model Saved")
    return best_model
...
# torch.save(infer_model.state_dict(), "./patchtst_submit_0916_ver1_final.pth")
...
infer_model.load_state_dict(torch.load("./patchtst_submit_0916_epoch10.pth"))
...
submit.iloc[:, 1:] = pred
submit.to_csv("./patchtst_submit_0916_ver1_epoch10.csv", index=False)
