...
def make_train_data_new(
    ...
    STEP_SIZE = 4  # 이 값을 본인의 환경에 맞게 조정
...
def train(model, optimizer, scheduler, train_loader, val_loader, device):
    ...
    for epoch in range(1, CFG["EPOCHS"] + 1):
        ...
        if best_loss > val_loss:
            best_loss = val_loss
            best_model = model
            torch.save(best_model.state_dict(), "./patchtst_submit_0916_ver3_epoch{}.pth".format(epoch))
            print("Model Saved")
    return best_model
...
torch.save(infer_model.state_dict(), "./patchtst_submit_0916_ver3_final.pth")
