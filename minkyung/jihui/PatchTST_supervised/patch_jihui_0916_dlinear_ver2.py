...
def train(model, optimizer, scheduler, train_loader, val_loader, device):
    ...
    for epoch in range(1, CFG["EPOCHS"] + 1):
        ...
        if best_loss > val_loss:
            best_loss = val_loss
            best_model = model
            torch.save(best_model.state_dict(), "/root/workspace/minkyung/ckpt/dlinear_submit_0917_ver3_epoch{}.pth".format(epoch))
            print("Model Saved")
    return best_model
...
# torch.save(infer_model.state_dict(), "/root/workspace/minkyung/ckpt/dlinear_submit_0917_ver3_final.pth")


# 저장된 파일 불러오기

infer_model = DLinear.Model(args).to(device)
infer_model = nn.DataParallel(infer_model)  # parallel mode

infer_model.load_state_dict(torch.load("/root/workspace/minkyung/ckpt/dlinear_submit_0917_ver3_final.pth"))
...
submit = pd.read_csv("/root/workspace/data/sample_submission.csv")
submit.iloc[:, 1:] = pred
submit.to_csv("/root/workspace/minkyung/ckpt/dlinear_submit_0917_ver3_final.csv", index=False)
