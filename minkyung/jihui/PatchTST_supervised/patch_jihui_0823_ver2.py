...
def make_train_data(
    ...
    input_data = np.empty(
        (
            ...
            train_size,
            1 + product.shape[1] + time_series.shape[0] + 2,
        )
    )
    ...
    for i in tqdm(range(num_rows), dynamic_ncols=True):
        encode_info_big = np.array(data.iloc[i, 0])  # 대분류
        # encode_info_small = np.array(data.iloc[i, 2])  # 소분류
...
def make_predict_data(data, train_size=CFG["TRAIN_WINDOW_SIZE"]):
    ...
    input_data = np.empty(
        (num_rows, train_size, 1 + product.shape[1] + time_series.shape[0] + 2)
    )
...
torch.save(infer_model.state_dict(), "./patchtst_submit_0823_ver2.pth")
...
infer_model.load_state_dict(torch.load("./patchtst_submit_0823_ver2.pth"))
...
submit.iloc[:, 1:] = pred
submit.to_csv("./patchtst_submit_0823_ver2.csv", index=False)
"""